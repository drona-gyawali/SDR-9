...
export default function FileUploader({ peer }: { peer: any }) {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 bg-transparent max-w-full overflow-hidden">
      {/* Premium Upload Zone */}
      <div
        ...
        className={`relative group cursor-pointer border-2 border-dashed rounded-4xl sm:rounded-[2.5rem] transition-all duration-500 p-8 sm:p-12 text-center
        ${isDragging
      ...
      >
        ...
        <div className="flex flex-col items-center">
          ...
          <h4 className="text-lg sm:text-xl font-bold text-zinc-800 dark:text-zinc-100 tracking-tight">Drop files here</h4>
          <p className="hidden sm:block text-zinc-500 dark:text-zinc-500 text-sm mt-2 max-w-xs mx-auto">
            Drag and drop or <span className="text-indigo-500 font-semibold hover:underline">browse</span>
          </p>
          <p className="sm:hidden text-indigo-500 text-sm font-semibold mt-1">Tap to upload</p>

          <div className="mt-4 sm:mt-6 flex items-center gap-2 px-3 py-1 bg-zinc-200/50 dark:bg-zinc-800/50 rounded-full">
            <span className="text-[10px] uppercase tracking-tighter text-zinc-500 dark:text-zinc-400 font-bold">
              {fileQueue.length} / {MAX_QUEUE_SIZE} Files
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="animate-in slide-in-from-bottom-4 duration-700">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 px-2 gap-4">
            <div>
              ...
              <p className="text-xs text-zinc-500 font-medium tracking-wide">Ready for P2P delivery</p>
            </div>
          ...
          <div className="space-y-3">
            {fileQueue.map((item) => (
              <div
                key={item.id}
                className="group relative flex items-center justify-between p-3 sm:p-4 bg-white dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800/60 rounded-3xl sm:rounded-3xl hover:border-zinc-300 dark:hover:border-zinc-700 transition-all"
              >
                <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                  <div className={`p-2.5 sm:p-3 rounded-xl sm:rounded-2xl shrink-0 transition-all duration-500 ${item.status === 'complete'
                      ? 'bg-emerald-500/10 text-emerald-500'
                      : item.status === 'transferring'
                        ? 'bg-indigo-500/10 text-indigo-500'
                        : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-600'
                    }`}>
                    <FileText size={20} className="sm:w-5.5 sm:h-5.5" strokeWidth={1.5} />
                  </div>

                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-zinc-800 dark:text-zinc-200 truncate pr-2 tracking-tight">
                      {item.file.name}
                    </p>
                    <div className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold mt-0.5 uppercase tracking-wider">
                      <span className="text-zinc-400 dark:text-zinc-500 shrink-0">{formatSize(item.file.size)}</span>
                      <span className="text-zinc-300 dark:text-zinc-800">•</span>
                      <span className={`truncate ${item.status === 'complete' ? 'text-emerald-500' :
                ...
                <div className="flex items-center gap-2 sm:gap-4 shrink-0 ml-2">
                  {item.status === 'transferring' && (
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <div className="w-12 sm:w-20 h-1 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                        <div
                  ...
                  <button
                    ...
                    className={`p-3 sm:p-2 -mr-1 rounded-xl transition-all cursor-pointer touch-manipulation ${item.status === 'transferring'
                        ? 'opacity-0 scale-50 pointer-events-none'
                        : 'text-zinc-300 dark:text-zinc-700 hover:text-red-500 active:bg-red-50 dark:active:bg-red-500/10'
                      }`}
                  >
                    <X size={18} />
                  </button>
